...
export default function MasterBuildings() {
	...
	const addNewOccupancy = () => {
		if (newOccupancyClassification === "") message.error("Enter a valid input");
			...
			api
				.post("/dropdown/occupancyClassification", { new_value })
				.then((res) => {
					if (res.status === 200) {
	...
	const addNewTypeOfConstruction = () => {
		if (newTypeOfConstruction === "") message.error("Enter a valid input");
			...
			api.post("/dropdown/typeOfConstruction", { new_value }).then((res) => {
				if (res.status === 200) {
	...
	const addNewContractType = () => {
		if (newContractType === "") message.error("Enter a valid input");
			...
			api.post("/dropdown/contractType", { new_value }).then((res) => {
				if (res.status === 200) {
					getContractType();
					setNewContractType("");
	...
	const getTypeOfConstruction = () => {
		...
		api.get("/dropdown/typeOfConstruction").then((res) => {
			if (res.status === 200) {
	...
	const getContractType = () => {
		...
		api.get("/dropdown/contractType").then((res) => {
			if (res.status === 200) {
	...
	const saveRequiredFields = () => {
		...
		api
			.post("/dropdown/saveBuildingFields", { fields: requiredFields })
			.then((res) => {
			...
			<Row>
				...
				<Col span={6} style={{ padding: "10px" }}>
					...
					<select
						...
						style={{ width: "100%", borderColor: "lightgrey" }}
					>
						{hazardClassification.map((hazard) => (
